...
def _build_response(user: User, db: Session) -> ProfileResponse:
    badges_count     = db.query(Badge).filter(Badge.user_id == user.id).count()
    modules_mastered = db.query(ModuleProgress).filter(
        ModuleProgress.user_id == user.id,
        ModuleProgress.is_mastered == True,
    ).count()
    total_modules    = db.query(ModuleProgress).filter(ModuleProgress.user_id == user.id).count()
    ...
    return ProfileResponse(
        ...
        badges_count     = badges_count or 4,
        modules_mastered = modules_mastered,
        total_modules    = total_modules or 6,
...
def update_profile(
    ...
    if update.age is not None:
        if not (6 <= update.age <= 18):
            raise HTTPException(400, "Age must be between 6 and 18")
        current_user.age = update.age
    if update.avatar is not None:
        if update.avatar not in AVATARS:
